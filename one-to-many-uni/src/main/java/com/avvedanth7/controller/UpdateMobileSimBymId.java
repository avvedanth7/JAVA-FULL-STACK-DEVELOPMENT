package com.avvedanth7.controller;

import java.util.ArrayList;
import java.util.List;

import com.avvedanth7.dao.MobileDao;
import com.avvedanth7.dto.Mobile;
import com.avvedanth7.dto.Sim;

public class UpdateMobileSimBymId {
	public static void main(String[] args) {
		Mobile mobile = new Mobile();
		mobile.setName("i Phone 14 plus");
		mobile.setBrand("Apple");
		mobile.setPrice(100000);
		Sim sim1 = new Sim();
		sim1.setProvider("Airtel");
		sim1.setGeneration("5G");

		Sim sim2 = new Sim();
		sim2.setProvider("Jio");
		sim2.setGeneration("5G");

		List<Sim> sims = new ArrayList<Sim>();

		sims.add(sim1);
		sims.add(sim2);

		mobile.setSims(sims);

		MobileDao dao = new MobileDao();
		boolean updated = dao.updateMobileSimBymId(5, mobile);
		
		if(updated) {
			System.out.println("Mobile Updated Successfully");
		}else {
			System.out.println("Could not find the mobile with specified id");
		}

	

	}

}
